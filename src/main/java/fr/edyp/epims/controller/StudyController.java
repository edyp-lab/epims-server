...
/**
 * Controller that handles API operations for managing studies within the system.
 * Provides endpoints for creating studies, changing study status and comments,
 * and adding members to a study.
 */
...
public class StudyController {
    ...
    public ResponseEntity<StudyJson> addStudy(@RequestBody StudyJson studyJson) {
        ...
        if (projectId != -1) {
            ...
            if (projectOpt.isEmpty()) {
                throw new EpimServerException(EpimsErrorCode.PROJECT_NOT_FOUND, "Project ID: " + projectId);
            }
        ...
        if (actorOpt.isEmpty()) {
            throw new EpimServerException(EpimsErrorCode.ACTOR_NOT_FOUND, "Actor Key: " + studyJson.getActorKey());
        }
        ...
        Optional<Study> existingStudyByNomenclature = studyRepository.findByNomenclatureTitle(studyJson.getNomenclatureTitle());
        if (existingStudyByNomenclature.isPresent()) {
            throw new EpimServerException(EpimsErrorCode.DUPLICATE_NOMENCLATURE,
                    "A study with nomenclature title '" + studyJson.getNomenclatureTitle() + "' already exists");
        }

        Optional<Study> existingStudyByTitle = studyRepository.findByTitle(studyJson.getTitle());
        if (existingStudyByTitle.isPresent()) {
            throw new EpimServerException(EpimsErrorCode.DUPLICATE_TITLE,
                    "A study with title '" + studyJson.getTitle() + "' already exists");
        }
    ...
    public ResponseEntity<StudyJson> studyChangeStatus(@RequestBody String status, @PathVariable("studyId") int studyId) {
        ...
        if (s.isEmpty()) {
            throw new EpimServerException(EpimsErrorCode.STUDY_NOT_FOUND, "Study Key: " + studyId);
        }
    ...
    public ResponseEntity<List<StudyJson>> getAllStudies() {
        List<Study> studies = new ArrayList<>(studyRepository.findAll());
